...
import { getAllDebatesAction } from "./debates/_actions/debates.query.action";

export default async function HomePage() {
  const debates = await getAllDebatesAction();

  const now = new Date();

  const liveDebates = debates.filter(debate => {
    const start = new Date(debate.startTime);
    const end = new Date(start.getTime() + debate.duration * 60000);
    return start <= now && now <= end;
  });

  const upcomingDebates = debates.filter(d => new Date(d.startTime) > now);
          ...
          <div className="grid md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-muted-foreground font-serif">
                <li><a href="#">Live Debates</a></li>
                <li><a href="#">Apply to Debate</a></li>
